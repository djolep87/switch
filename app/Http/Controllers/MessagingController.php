...
class MessagingController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($conversations as $otherUserId => $conversationMessages) {
            ...
            if ($messageWithOffer && $messageWithOffer->offer) {
                // Show the ad that belongs to the OTHER user (the one whose name is displayed)
                if ($messageWithOffer->offer->user_id == $otherUserId) {
                    // Other user is the offer creator - show what current user is offering (sendproduct)
            ...
            $messages[] = [
                ...
                'subject' => $latestMessage->offer ? $latestMessage->offer->product->name ?? 'Razmena' : 'Poruka',
    ...
    public function show($id)
    {
        ...
        if ($messageWithOffer && $messageWithOffer->offer) {
            // Show the ad that belongs to the OTHER user (the one whose name is displayed)
            if ($messageWithOffer->offer->user_id == $id) {
                // Other user is the offer creator - show what current user is offering (sendproduct)
                $adTitle = $messageWithOffer->offer->sendproduct ? $messageWithOffer->offer->sendproduct->name : 'Razmena';
